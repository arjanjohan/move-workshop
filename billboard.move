module deployment_addr::billboard {
    ...
    public entry fun add_message(sender: &signer, message: String) acquires Billboard {
        ...
        let billboard = borrow_global_mut<Billboard>(@deployment_addr);
    ...
    inline fun only_owner(owner: &signer) {
        assert!(signer::address_of(owner) == @deployment_addr, error::permission_denied(ENOT_OWNER));
    }

    #[view]
    public fun get_messages(): vector<Message> acquires Billboard {
        let billboard = borrow_global<Billboard>(@deployment_addr);
